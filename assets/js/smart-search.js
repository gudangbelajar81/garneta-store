...
(function() {
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', function(e) { if (!e.target.closest('.smart-search-wrapper')) { closeDropdown(); } document.querySelectorAll('.kebab-dropdown').forEach(d => { if (!d.contains(e.target) && !e.target.matches('button')) d.classList.add('hidden'); }); });
