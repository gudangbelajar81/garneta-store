window.showToast = function(msg, icon = "info") { if (typeof Swal !== "undefined") { Swal.fire({toast: true, position: "top-end", showConfirmButton: false, timer: 3000, title: msg, icon: icon}); } else { alert(msg); } };
    ...
    async function gas(action, payload = {}, silentAuthError = false) {
      ...
      if (!response.ok) {
        if ((response.status === 401 || response.status === 400) && result?.message) {
           ...
           if (isInvalidSession) {
               ...
               if (!silentAuthError) {
                   ...
                   if (typeof window.showToast === 'function') window.showToast("Sesi berakhir, silakan login kembali.", "error");
               }
           } else if (msg.includes("khusus Super Admin") && !silentAuthError && !["bootstrap", "sync", "list", "getSetting"].includes(action)) {
               if (typeof window.showToast === 'function') window.showToast("Akses ditolak: Fitur ini khusus Super Admin.", "error");
           }
           
           if (msg.includes("Akses ditolak") || msg.includes("login terlebih dahulu")) {
              throw new Error(msg);
           }
        }
...
Berdasarkan rincian di atas, untuk gajian periode ini kasbonnya mau *Dipotong Full (Lunas)* atau mau *Dicicil Sebagian* dulu?`;
  ...
  if (formPayroll) {
    formPayroll.onsubmit = async (e) => {
      ...
      const views = { dashboard, "neural-hub": neuralHub, barang, supplier, pembelian, ngitung, riwayat, kalkulator, penjualan, laporan, statistik, audit, users, gaji, settings, kasbon , ppob, kentang: typeof kentang !== 'undefined' ? kentang : () => 'Loading...'};
    ...
    function pembelian() {
      const workspaces = [
        { id: 'notepad', icon: '📝', label: 'Notepad Pintar' },
      ...
      switch(activeWorkspace) {
        case 'notepad':
          workspaceContent = `<div class="workspace-content">
            <div class="card">
              <h3 style="color:var(--garneta-cyan);">📝 Notepad Belanja (Estimasi Total)</h3>
              <p class="muted" style="margin-top:0;">Ketik daftar belanjaan per baris (Contoh: <strong>Rose beras 10</strong>). Sistem akan mengkalkulasi estimasi harga dari database barang.</p>
              <textarea id="pembelian-notepad-input" class="input-area expandable" placeholder="Rose beras 10\nLM 5d\nCakra 5d\nTapioka rose brand 5\nGA 10" style="background-image: repeating-linear-gradient(transparent, transparent 27px, rgba(255,255,255,0.05) 28px); line-height: 28px; padding-top: 6px; min-height: 250px; font-family: monospace; font-size:1.05rem; width:100%; border:1px solid var(--border); border-radius:8px; padding-left:12px; background-color: var(--bg-card); color: var(--text-color);" oninput="window.kalkulasiNotepadPembelian()"></textarea>
              <div class="actions" style="margin-top:12px; justify-content: flex-end;">
                <button class="btn primary" onclick="window.kalkulasiNotepadPembelian()">🧮 Hitung Estimasi</button>
                <button class="btn danger" onclick="document.getElementById('pembelian-notepad-input').value=''; document.getElementById('pembelian-notepad-result').innerHTML='';">🗑️ Kosongkan</button>
              </div>
              <div id="pembelian-notepad-result" style="margin-top:20px;"></div>
            </div>
          </div>`;
          break;
        case 'search':
...
${tab === "bluetooth" ? `
    ...
    function productForm() {
      const cats = window.getCategoriesList();
...
Payung, Tepung, sak, 25, 170000, 8500"></textarea>
    ...
    function priceWithUnit(namePrice, nameUnit, label, unitPlaceholder, isEcer, marginBaseName = null) {
...
window.kalkulasiNotepadPembelian = function() {
